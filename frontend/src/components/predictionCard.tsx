...
export default function PredictionCard(props: PredictionCardProps) {
  const {
    league, league_name, country, date,
    home_team, away_team, home_logo, away_logo,
    predictions, odds, top_scorers,
  } = props;
    ...
    <div className="p-5 rounded-2xl border border-gray-800 bg-gray-950 hover:border-green-500 transition flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="rounded-xl bg-gray-900 border border-gray-800 p-3">
          ...
          <div className="text-sm text-white">
            {dcLabel(predictions?.double_chance?.class)} <span className="text-gray-400 ml-1">({toPct(prob(predictions?.double_chance))})</span>
          </div>
        </div>
        <div className="rounded-xl bg-gray-900 border border-gray-800 p-3">
          <div className="text-xs text-gray-400">Over 2.5</div>
          <div className="text-sm text-white">
            {predictions?.over_2_5?.class ? "Sim" : "Não"} <span className="text-gray-400 ml-1">({toPct(prob(predictions?.over_2_5))})</span>
          </div>
        </div>
        <div className="rounded-xl bg-gray-900 border border-gray-800 p-3">
          <div className="text-xs text-gray-400">Over 1.5</div>
          <div className="text-sm text-white">
            {predictions?.over_1_5?.class ? "Sim" : "Não"} <span className="text-gray-400 ml-1">({toPct(prob(predictions?.over_1_5))})</span>
          </div>
        </div>
        <div className="rounded-xl bg-gray-900 border border-gray-800 p-3 col-span-2">
          <div className="text-xs text-gray-400">BTTS</div>
          <div className="text-sm text-white">
            {predictions?.btts?.class ? "Sim" : "Não"} <span className="text-gray-400 ml-1">({toPct(prob(predictions?.btts))})</span>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <ul className="text-sm text-white space-y-1">
              {(top_scorers ?? []).slice(0, 5).map((sc, idx) => (
                <li key={idx} className="flex justify-between">
                  <span>{sc.player} <span className="text-gray-400">({sc.team})</span></span>
