...
// 0=1X, 1=12, 2=X2 (se precisares noutro lugar)
type DCClass = 0 | 1 | 2;
...
export default function HomeClient() {
  ...
  // Conjunto de IDs permitidos (para filtrar previsões)
  const allowedLeagueIds = useMemo(
    () => new Set<string>(backendLeagues.map((x) => String(x.id))),
    [backendLeagues]
  );

  // Dropdown: só ligas curadas + "Todas"
  ...
  const loadMainData = useCallback(async () => {
    ...
    try {
      ...
      // ✅ Type guard para StatsType | null
      const statsOk =
        statsData &&
        typeof statsData === "object" &&
        Object.keys(statsData as Record<string, unknown>).length > 0
  ...
  }, [allowedLeagueIds, selectedDateISO, selectedLeague]);
  ...
  return (
    <div className="min-h-screen container mx-auto px-4 py-8 md:py-16">
      ...
      <main className="space-y-12 md:space-y-16">
        ...
        {/* Previsões (filtradas pelas ligas curadas, se houver) */}
        {Array.isArray(predictions) && predictions.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {predictions.map((p: any) => {
              ...
              return (
                <div
                  key={String(p.match_id ?? p.fixture_id ?? `${p.home_team}-${p.away_team}-${p.date}`)}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className={`rounded-xl border p-3 ${tileClass(prWinner, isTop("winner"))}`}>
                      <div className="text-xs text-gray-400 flex items-center justify-between">
                        <span>Winner</span>
                        {isTop("winner") && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/80 text-white">TOP</span>
                        )}
                      </div>
