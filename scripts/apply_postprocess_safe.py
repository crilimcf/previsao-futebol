...
OUT = Path("tmp/postprocess_applied.json")
AUDIT = Path("tmp/postprocess_applied_audit_by_league.csv")
...
def main():
    ...
    out = []
    stats = defaultdict(lambda: {"count":0, "final_over_ge_0_99":0, "final_over_le_0_01":0, "raw_over_ge_0_99":0, "raw_over_le_0_01":0})
    ...
    for p in preds:
        ...
        try:
            ...
            if final_over is not None:
                if final_over >= 0.99:
                    stats[lid]["final_over_ge_0_99"] += 1
                if final_over <= 0.01:
                    stats[lid]["final_over_le_0_01"] += 1
    ...
    with AUDIT.open("w", encoding="utf-8") as fh:
        ...
        for k,v in sorted(stats.items(), key=lambda x: -x[1]["count"]):
            fh.write(f"{k},{v['count']},{v['final_over_ge_0_99']},{v['final_over_le_0_01']},{v['raw_over_ge_0_99']},{v['raw_over_le_0_01']}\n")

    print(f"[ok] applied postprocess to {len(out)} items. outputs: {OUT}, {AUDIT}")
