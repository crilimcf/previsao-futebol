...
# ===========================================
# 🔌 Redis (somente token - compatível com Upstash)
# ===========================================
redis = None
if REDIS_URL and REDIS_TOKEN:
    ...
    logging.warning("⚠️ Redis não configurado — variáveis REDIS_URL e REDIS_TOKEN ausentes.")
...
def _call_api_football(endpoint, params):
    if not API_KEY:
        logging.error("❌ API-Football KEY não definida.")
        return {}
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            if data.get("errors"):
                logging.warning(f"⚠️ Erros na resposta da API: {data['errors']}")
            return data
        else:
...
# ===========================================
# 📅 Busca próximos jogos (com fallback automático)
# ===========================================
def fetch_today_matches():
    logging.info(f"🌍 API-Football ativo | Época {API_SEASON}")
    all_fixtures = []

    for season in [API_SEASON, str(int(API_SEASON) - 1)]:
        logging.info(f"🔎 A buscar próximos 50 jogos (época {season})...")
        data = _call_api_football("fixtures", {"next": 50, "season": season})
        ...
        if data and data.get("response"):
            ...
            break  # Sai do loop se encontrou jogos
    ...
    if redis:
        try:
            ...
            logging.info("📦 Redis atualizado com previsões e metadados.")
        except Exception as e:
