...
def get_market_odds(fixture_id: int) -> Dict[str, Any]:
    """
    Tenta via proxy /odds; se não houver, tenta API direta.
    Agrega por mediana. Retorna dict com chaves 'winner', 'over_2_5', 'btts'.
    """
    ...
    if payload is None:
        ...
        # a api_get já retorna o 'response', embrulhamos para formato idêntico ao proxy
            ...
            for bm in item.get("bookmakers", []) or []:
                for bet in bm.get("bets", []) or []:
                    ...
                    if name in ("match winner", "1x2"):
                        ...
                        for k,v in w.items():
                            if v is not None:
                                # mantemos a mediana global (recolhe em listas e tiramos mediana no fim)
                                pass
    ...
    # Volta a calcular winner pela mediana final (segura)
    # (acima deixámos média incremental; aqui só garantimos range)
...
def build_prediction_from_fixture(fix: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    try:
        ...
        # ----- odds modeladas (implied) como base -----
        odds_map = {
            "winner": {
        ...
        # ----- tenta substituir por odds reais de mercado -----
...
def collect_fixtures(days: int = 3) -> List[Dict[str, Any]]:
    ...
    for d in range(days):
        ...
        if pj and isinstance(pj.get("response"), list):
            ...
            logger.warning(f"Sem fixtures via proxy para {iso}.")
        time.sleep(0.2)
    if not fixtures:
