...
import os, json, math, time, logging, re, statistics as stats
...
# ---------- HTTP ----------
def proxy_get(path: str, params: Optional[Dict[str, Any]] = None, timeout: int = 12) -> Optional[Dict[str, Any]]:
...
def api_get(endpoint: str, params: Optional[Dict[str, Any]] = None, timeout: int = 12) -> Any:
    if not API_KEY:
...
def poisson_score_probs(lambda_home: float, lambda_away: float, max_goals: int = 6) -> List[List[float]]:
    ...
    if s > 0: mat = [[x / s for x in r] for r in mat]
    return mat
...
def over_under_prob_from_matrix(mat: List[List[float]], line: float) -> Tuple[float, float]:
    n = len(mat); p_over = 0.0
    for h in range(n):
        for a in range(n):
            if (h + a) > line: p_over += mat[h][a]
...
def _median_or_none(vals: List[float]) -> Optional[float]:
    ...
    try: return round(float(stats.median(vals)), 2)
    except Exception: return None

def _parse_over_under(bet: Dict[str, Any], line: float) -> Tuple[Optional[float], Optional[float]]:
    over_list: List[float] = []; under_list: List[float] = []
    for v in bet.get("values", []) or []:
        label = (v.get("value") or "").lower().replace(",", ".").strip()
        odd_raw = v.get("odd"); 
        if odd_raw is None: continue
        try: odd = float(str(odd_raw).replace(",", "."))
        except Exception: continue
        if label == f"over {line}".lower(): over_list.append(odd)
        elif label == f"under {line}".lower(): under_list.append(odd)
...
def _parse_match_winner(bet: Dict[str, Any]) -> Dict[str, Optional[float]]:
    ...
    for v in bet.get("values", []) or []:
        ...
        if label in ("home", "1"): buff["home"].append(odd)
        elif label in ("draw", "x"): buff["draw"].append(odd)
        elif label in ("away", "2"): buff["away"].append(odd)
    for k in out.keys(): out[k] = _median_or_none(buff[k])
    return out

def _parse_btts(bet: Dict[str, Any]) -> Dict[str, Optional[float]]:
    out = {"yes": None, "no": None}; buff = {"yes": [], "no": []}
    for v in bet.get("values", []) or []:
        label = (v.get("value") or "").lower().strip()
        odd_raw = v.get("odd")
        if odd_raw is None: continue
        ...
        if "yes" in label: buff["yes"].append(odd)
        elif "no" in label: buff["no"].append(odd)
    out["yes"] = _median_or_none(buff["yes"]); out["no"] = _median_or_none(buff["no"])
    return out
...
def get_market_odds(fixture_id: int) -> Dict[str, Any]:
    ...
    out = {"winner": {"home": None, "draw": None, "away": None},
            ...
            for bm in item.get("bookmakers", []) or []:
                for bet in bm.get("bets", []) or []:
                    ...
                    if "over/under" in name or "under/over" in name:
                        ...
                        if over  is not None: out["over_2_5"]["over"]  = over  if out["over_2_5"]["over"]  is None else round((out["over_2_5"]["over"]  + over)/2, 2)
                        if under is not None: out["over_2_5"]["under"] = under if out["over_2_5"]["under"] is None else round((out["over_2_5"]["under"] + under)/2, 2)
...
def compute_lambdas(stats_home: Dict[str, Any], stats_away: Dict[str, Any]) -> Tuple[float, float]:
    def _f(v, d=1.0):
        try:
            ...
            return float(d)
    avg_goals_home = _f(stats_home.get("goals", {}).get("for", {}).get("average", {}).get("home", 1.2))
    avg_goals_away = _f(stats_away.get("goals", {}).get("for", {}).get("average", {}).get("away", 1.1))
    avg_conc_home  = _f(stats_home.get("goals", {}).get("against", {}).get("average", {}).get("home", 1.0))
    avg_conc_away  = _f(stats_away.get("goals", {}).get("against", {}).get("average", {}).get("away", 1.0))
...
def implied_odds(p: float) -> float:
    p = clamp_prob(p); return round(1.0 / p, 2)

def pick_dc_class(ph: float, pd: float, pa: float) -> Tuple[int, float]:
    opts = [(0, ph+pd), (1, ph+pa), (2, pd+pa)]
    return max(opts, key=lambda t: t[1])

# ---------- Filtros de selecção A ----------
_UXX_RE = re.compile(r"\bU(16|17|18|19|20|21|22|23)\b", re.I)

INTL_ALLOW_PATTERNS = [
    "world cup", "wc qualification", "world cup qualification",
    "euro", "uefa euro", "european championship", "european qualifiers",
    "nations league", "copa america", "africa cup of nations", "afcon",
    "african cup", "asian cup", "gold cup", "concacaf nations league",
    "ofc nations", "friendly", "friendlies"
]

def _is_women(s: str) -> bool:
    s = s.lower()
    return "women" in s or "femin" in s or "fémin" in s or "(w)" in s

def _is_youth(s: str) -> bool:
    return bool(_UXX_RE.search(s))

def _looks_international_A(league_name: str, country: str) -> bool:
    ln = (league_name or "").lower()
    if any(p in ln for p in INTL_ALLOW_PATTERNS):
        # excluir explicitamente variantes women/youth
        if _is_women(ln) or _is_youth(ln): 
            return False
        return True
    # também tratamos ligas com country tipo "World", "Europe", etc.
    geo = (country or "").lower()
    return geo in {"world","europe","africa","asia","south america","north america","oceania"}

def _allow_fixture(fx: Dict[str, Any]) -> bool:
    league = fx.get("league", {}) or {}
    teams  = fx.get("teams", {}) or {}
    hn = (teams.get("home", {}) or {}).get("name") or ""
    an = (teams.get("away", {}) or {}).get("name") or ""
    ln = league.get("name") or ""
    cn = league.get("country") or ""

    blob = f"{hn} {an} {ln}".lower()
    if _is_women(blob) or _is_youth(blob): 
        return False  # corta Women / U-XX

    # Modo inclusivo: aceitamos clubes + selecção A
    # (se for competição internacional, só aceita A)
    if _looks_international_A(ln, cn):
        return True
    return True  # clubes → true

# ---------- Core ----------
def build_prediction_from_fixture(fix: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    try:
        if not _allow_fixture(fix):
            return None
        ...
        if fix_id:
            ...
            if any(isinstance(v,(int,float)) for v in mkt["winner"].values()):
                for k in ("home","draw","away"):
                    if isinstance(mkt["winner"].get(k),(int,float)):
                        odds_map["winner"][k] = mkt["winner"][k]; odds_source = "market"
            if isinstance(mkt["over_2_5"].get("over"),(int,float)) and isinstance(mkt["over_2_5"].get("under"),(int,float)):
                ...
                odds_map["over_2_5"]["under"] = mkt["over_2_5"]["under"]; odds_source = "market"
            if isinstance(mkt["btts"].get("yes"),(int,float)) and isinstance(mkt["btts"].get("no"),(int,float)):
                odds_map["btts"]["yes"] = mkt["btts"]["yes"]; odds_map["btts"]["no"]  = mkt["btts"]["no"]; odds_source = "market"

        pred_score = top3[0]["score"] if top3 else "1-1"
        try:
            hs, as_ = pred_score.split("-"); ps_obj = {"home": int(hs), "away": int(as_)}
        except Exception:
...
# ---------- Fixtures & pipeline ----------
def _fetch_fixtures_one_day(iso: str) -> List[Dict[str, Any]]:
    """Tenta proxy -> fallback API direta. Não filtra aqui."""
    out: List[Dict[str, Any]] = []
    # proxy
    pj = proxy_get("/fixtures", {"date": iso, "season": SEASON})
    if pj and isinstance(pj.get("response"), list):
        out.extend(pj["response"])
    # fallback api direta
    if not out:
        resp = api_get("fixtures", {"date": iso, "season": SEASON})
...
def collect_fixtures(days: int = 3) -> List[Dict[str, Any]]:
    ...
    for d in range(days):
        ...
        raw = _fetch_fixtures_one_day(iso)
        logger.info(f"[fixtures] {iso}: recebidos={len(raw)}")
        for fx in raw:
            fid = ((fx.get("fixture") or {}).get("id")) or None
            if fid and fid in seen: 
                continue
            if _allow_fixture(fx):
                fixtures.append(fx)
                if fid: seen.add(fid)
        logger.info(f"[fixtures] {iso}: após filtros={sum(1 for _ in raw if _allow_fixture(_))}")
...
def fetch_and_save_predictions() -> Dict[str, Any]:
    ...
    logger.info(f"{len(fixtures)} fixtures após filtros (clubes + selecção A).")
